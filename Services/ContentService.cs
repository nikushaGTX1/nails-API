...
public sealed class ContentService(NailsDbContext db) : IContentService
{
    public async Task<SiteContentDto> GetAsync(CancellationToken cancellationToken = default)
    {
        ...
        return new SiteContentDto
        {
            ...
            Services = await db.Services.AsNoTracking().OrderBy(x => x.SortOrder).Select(x => new ServiceDto { Id=x.Id, Name=x.Name, Description=x.Description, Price=x.Price, CategoryId=x.CategoryId, GroupLabel=x.GroupLabel, SubgroupLabel=x.SubgroupLabel }).ToListAsync(cancellationToken),
